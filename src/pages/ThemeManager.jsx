...
const getToken = () => localStorage.getItem('ilnaz-token');
  ...
  const deletePublicTheme = async (themeId, theme) => {
    const id = theme?.id || theme?._id || themeId;
    console.log('Deleting theme, id:', id);
    ...
    try {
      ...
      const res = await fetch(`${getServerUrl()}/api/themes/public/${id}`, {
        method: 'DELETE',
