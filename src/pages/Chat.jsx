...
const Chat = () => {
  ...
  const startAudioLevelMonitoring = () => {
    const checkLevels = () => {
      const newSpeaking = {};
      for (const uid in analyzersRef.current) {
        const analyser = analyzersRef.current[uid];
        if (analyser) {
          const data = new Uint8Array(analyser.frequencyBinCount);
          analyser.getByteFrequencyData(data);
          const avg = data.reduce((a, b) => a + b, 0) / data.length;
          newSpeaking[uid] = avg > 15;
        }
      }
  ...
  const setupPeerConnection = (pc, remoteUserId, convId, stream) => {
    ...
    pc.ontrack = (event) => {
      ...
      if (remoteStream.getAudioTracks().length > 0) {
        setRemoteAudioStreams(prev => ({ ...prev, [remoteUserId]: remoteStream }));
        setupAudioAnalyzer(remoteUserId, remoteStream);
        startAudioLevelMonitoring();
      }
      if (remoteStream.getVideoTracks().length > 0) {
        setRemoteVideoStreams(prev => ({ ...prev, [remoteUserId]: remoteStream }));
      }
    };
  ...
  const processPendingOffers = useCallback(() => {
    ...
    for (const fromId in pending) {
      const offerData = pending[fromId];
      delete pendingOffersRef.current[fromId];
      handlePeerOffer(offerData);
    }
  }, []);
    ...
    if (isVideoOn) {
      ...
      try {
        ...
        Object.values(peerConnectionsRef.current).forEach(pc => {
          ...
          if (!hasVideo) pc.addTrack(videoTrack);
        });
      } catch (err) {
  ...
  const toggleScreenShare = useCallback(async () => {
    ...
    if (isScreenSharing) {
      ...
      try {
        const nv = await navigator.mediaDevices.getUserMedia({ video: true });
        localStreamRef.current.addTrack(nv.getVideoTracks()[0]);
      } catch (err) {}
      setIsScreenSharing(false);
      ...
      try {
        ...
        if (ov) { localStreamRef.current.removeTrack(ov); ov.stop(); }
        localStreamRef.current.addTrack(dt);
        setIsScreenSharing(true);
        setIsVideoOn(false);
        if (localVideoRef.current) localVideoRef.current.srcObject = localStreamRef.current;
        Object.values(peerConnectionsRef.current).forEach(pc => {
          const vs = pc.getSenders().find(s => s.track?.kind === 'video');
          if (vs) vs.replaceTrack(dt);
        });
        dt.onended = () => toggleScreenShare();
      } catch (err) {
  ...
  useEffect(() => {
    ...
    socket.on('user-typing', ({ conversationId, userId }) => {
      if (conversationId === activeConvRef.current?.id && userId !== userRef.current?.id) {
        ...
        typingTimeoutRef.current = setTimeout(() => setTypingUsers(prev => { const n = { ...prev }; delete n[userId]; return n; }), 3000);
      }
    });
  ...
  useEffect(() => {
    ...
    loadMessages(activeConv.id).then(msgs => {
      ...
      if (msgs?.length > 0) {
        markAsRead(activeConv.id, msgs[msgs.length - 1].id).then(() => refreshConversations());
      }
    });
  }, [activeConv?.id]);
  ...
  const loadMessages = async (convId) => { const msgs = await getMessages(convId); setMessages(msgs); return msgs; };
  const handleSelectConversation = (conv) => { setActiveConv(conv); };
  const handleCreateChat = async () => { await loadConversations(); if (activeConv) { const d = await getConversation(activeConv.id); setActiveConv({ ...activeConv, ...d, members: d?.members }); } };
  const handleSendMessage = () => {};
  const handleEditGroupUpdated = (c) => { setActiveConv(prev => ({ ...prev, ...c })); loadConversations(); };
  const handleDeleteGroup = async () => { if (!activeConv || !user) return; if (!confirm('Удалить эту группу?')) return; const r = await deleteConversation(activeConv.id); if (r?.success) { setActiveConv(null); setMessages([]); loadConversations(); } };
  const handleImageClick = (url) => { setExpandedImage(url); };

  const callMembersObjects = callJoinedMembers.map(id => {
    const m = getMemberById(id);
    return m || { id, nickname: id === user?.id ? 'Вы' : id, avatar: null };
  });
  ...
  return (
    <div className="chat-page fade-in">
      ...
      <div className="chat-main">
        {!activeConv ? (
          ...
          <>
            ...
            {showIncomingCall && (
              <div className="call-overlay call-incoming-overlay">
                <div className="call-incoming-panel">
                  ...
                  <div className="call-incoming-title">{currentCallConv ? (currentCallConv.type === 'group' ? currentCallConv.name : currentCallConv.other_user_name) : 'Входящий звонок'}</div>
                  <div className="call-incoming-subtitle">{getMemberById(callInitiatorId)?.nickname || 'Участник'} звонит...</div>
              ...
              <div className="call-overlay">
                <div className="call-panel">
                  <div className="call-header">
                    <div className="call-title">{currentCallConv ? (currentCallConv.type === 'group' ? currentCallConv.name : currentCallConv.other_user_name) : 'Звонок'}</div>
                    <div className="call-timer">{formatDuration(callDuration)}</div>
                  </div>
                  <div className="call-members-grid">
                    {callMembersObjects.map(member => {
                      const isSelf = member.id === user?.id;
                      const isSp = speakingUsers[member.id];
                      const isRm = !isSelf && mutedUsers[member.id];
                      return (
                        <div key={member.id} className={`call-member-avatar ${isSp ? 'speaking' : ''} ${isRm ? 'muted' : ''}`}>
                          <div className="call-member-avatar-inner">
                            {member.avatar ? <img src={member.avatar} alt="" /> : <span>{member.nickname?.charAt(0).toUpperCase()}</span>}
                            {remoteVideoStreams[member.id] && (
                              <video className="call-member-video" autoPlay playsInline ref={el => { if (el) el.srcObject = remoteVideoStreams[member.id]; }} />
                            )}
                          </div>
                          <div className="call-member-name">{member.nickname}</div>
                          {isRm && (
                            <div className="call-member-muted-badge">
                              <svg viewBox="0 0 24 24" fill="currentColor" width="10" height="10"><path d="M19 11h-1.7c0 .74-.16 1.43-.43 2.05l1.23 1.23c.56-.98.9-2.09.9-3.28zm-4.01.97l2.04 2.04.97-.97-3-3-.97.93h1zm-4.02.97c0 1.19.34 2.3.9 3.28l1.23-1.23c-.27-.62-.43-1.31-.43-2.05H7l-.97.93 2.04 2.04-.97.97-3-3-.97.93 3.96 3.96c-.79.47-1.69.76-2.66.85V21h6.04c-.97-.09-1.87-.38-2.66-.85L3.29 17.29a.996.996 0 0 1-.29-.71c0-.28.11-.53.29-.71l2.48-2.48c.18-.18.43-.29.71-.29.27 0 .52.1.7.28l1.23 1.23c-.27-.62-.43-1.31-.43-2.05H5v-2h2V8.67l2 2V11h1v1.97z"/></svg>
                            </div>
                          )}
                        </div>
                      );
                  ...
                  <div className="call-video-area">
                    {Object.entries(remoteVideoStreams).map(([uid, stream]) => (
                      <div key={uid} className="remote-video-container">
                        <video ref={el => { if (el) el.srcObject = stream; }} autoPlay playsInline className="remote-video" />
                        <div className="remote-video-label">{getMemberById(uid)?.nickname || uid}</div>
                      </div>
                    ))}
                    {Object.keys(remoteVideoStreams).length === 0 && (
                      <div className="call-no-video">
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" width="48" height="48"><path d="M12 1a3 3 0 0 0-3 3v8a3 3 0 0 0 6 0V4a3 3 0 0 0-3-3z" /><path d="M19 10v2a7 7 0 0 1-14 0v-2" /><line x1="12" y1="19" x2="12" y2="23" /><line x1="8" y1="23" x2="16" y2="23" /></svg>
                        <p>Аудиозвонок</p>
                      </div>
                    )}
                    <video ref={localVideoRef} autoPlay playsInline muted className={`local-video ${isVideoOn || isScreenSharing ? 'visible' : 'hidden'}`} />
                  </div>
                  <div className="call-controls">
                    <button className={`call-btn ${isMuted ? 'call-btn-active' : ''}`} onClick={toggleMute} title={isMuted ? 'Включить микрофон' : 'Выключить микрофон'}>
                      {isMuted ? <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><line x1="1" y1="1" x2="23" y2="23" /><path d="M9 9v3a3 3 0 0 0 5.12 2.12M15 9.34V4a3 3 0 0 0-5.94-.6" /><path d="M17 16.95A7 7 0 0 1 5 12v-2m14 0v2c0 .76-.13 1.49-.35 2.17" /><line x1="12" y1="19" x2="12" y2="23" /><line x1="8" y1="23" x2="16" y2="23" /></svg>
                    ...
                    <button className={`call-btn ${isVideoOn ? 'call-btn-active' : ''}`} onClick={toggleVideo} title={isVideoOn ? 'Выключить камеру' : 'Включить камеру'}>
                      {isVideoOn ? <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polygon points="23 7 16 12 23 17 23 7" /><rect x="1" y="5" width="15" height="14" rx="2" ry="2" /></svg>
                        : <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M16 16v1a2 2 0 0 1-2 2H3a2 2 0 0 1-2-2V7a2 2 0 0 1 2-2h2m5.66 0H14a2 2 0 0 1 2 2v3.34l1 1L23 7v10" /><line x1="1" y1="1" x2="23" y2="23" /></svg>}
                    </button>
            ...
            {callActive && Object.entries(remoteAudioStreams).map(([uid, stream]) => (
              <audio key={uid} autoPlay playsInline ref={el => { if (el) el.srcObject = stream; }} />
            ))}

            <MessageInput conversationId={activeConv.id} onSend={handleSendMessage} />
          </>
        )}
      </div>
      ...
      {showNewChat && <NewChatModal onClose={() => setShowNewChat(false)} onCreate={handleCreateChat} />}
            ...
            <div className="members-list">
              {(activeConv.members || []).map(m => (
                <div className="member-item" key={m.id}>
                  <div className="member-item-avatar">{m.avatar ? <img src={m.avatar} alt="" /> : m.nickname?.charAt(0)}</div>
                  <span className="member-item-name">{m.nickname}</span>
                  <span className="member-item-status">{m.status === 'online' ? 'В сети' : 'Не в сети'}</span>
                </div>
              ))}
            </div>
      ...
      {showEditGroup && activeConv && (
        <EditGroupModal conversation={activeConv} onClose={() => setShowEditGroup(false)} onUpdated={handleEditGroupUpdated} />
      )}
